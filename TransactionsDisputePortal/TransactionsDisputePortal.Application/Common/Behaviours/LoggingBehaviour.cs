...
namespace TransactionsDisputePortal.Application.Common.Behaviours
{
        ...
        where TRequest : notnull
    {
        ...
        public async Task Process(TRequest request, CancellationToken cancellationToken)
        {
            ...
            {
                _logger.LogInformation("TransactionsDisputePortal Request: {Name} {@UserId} {@UserName} {@Request}", requestName, user?.Id, user?.Name, request);
            }
            else
            {
                _logger.LogInformation("TransactionsDisputePortal Request: {Name} {@UserId} {@UserName}", requestName, user?.Id, user?.Name);
            }
        }
    }
}